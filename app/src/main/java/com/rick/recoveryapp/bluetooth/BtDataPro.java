...
//        }
        ...
        if (message.length() > 0) {
            ...
            try {
                Log.d("hexStr2Bytes", "--- message  ---" + message);
                send = hexStr2Bytes(message);
                Log.d("hexStr2Bytes", "--- send  ---" + send.toString());
            } catch (Exception e) {
...
//                    Toast.LENGTH_SHORT).show();
    ...
    public void Processing(ArrayList<String> RXlist, String type) {
        ...
        switch (type) {
            ...
            //处理其他一般数据
            case "82":
                Log.d("BT_CONNECTED", "------1   setBlood  ----- " + RXlist.get(17));
                UploadData uploadData = Results_Data(RXlist);
//                Log.d("血氧", "------   血氧     ----- " + RXlist.get(18));
//                ToastUtils.toast(" ----------血氧------  " + RXlist.get(18));
                ...
                LiveEventBus.get("BT_PROTOCOL")
                        .post(poolMessage);
//                ToastUtils.toast("82=      " + poolMessage.getObjectJson());
//                Log.d("BT_CONNECTED", "----   1     poolMessage   ----" + RXlist.get(17));
                break;
            ...
            case "83":
                ...
                if (ecgData != null) {
                    ...
                    LiveEventBus.get("BT_PROTOCOL")
                            .post(poolMessage);
//                    Log.d("BT_CONNECTED", "----   2     poolMessage   ----" + RXlist.get(17));
                } else {
                    poolMessage.setObjectJson(null);
                    poolMessage.setObjectName("");
                    poolMessage.setState(false);
...
//                ToastUtils.toast("83=      " + poolMessage.getObjectJson());
                break;
            ...
            case "85":
                ...
                poolMessage.setObjectName(MAC_ANSWER);
                poolMessage.setState(true);
                //  LocalConfig.poolMessage = poolMessage;
                LiveEventBus.get("BT_PROTOCOL")
                        .post(poolMessage);
                Log.d("hexStr2Bytes", "---   85  连接成功 ---" );
//                Log.d("BT_CONNECTED", "----   3     poolMessage   ----" + RXlist.get(17));
                break;
            ...
            case "84":
                ...
                LiveEventBus.get("BT_PROTOCOL")
                        .post(poolMessage);
//                Log.d("BT_CONNECTED", "----   4     poolMessage   ----" + RXlist.get(17));
                Log.d("hexStr2Bytes", "---    84 连接成功 ---" );
                break;
    ...
    public PoolMessage U3DProcessing(ArrayList<String> RXlist) {
        ...
        if (type.equals("81")) {
            ...
            Log.d("BT_CONNECTED", "------2   setBlood  ----- " + RXlist.get(17));
            UploadData uploadData = Results_Data(RXlist);
...
//        String[] split = hexstr.toString().split(" ");
        ...
        MassageStr = '\n' + "接收到数据：" + strbuf + '\n' + "设备ID号： " + DeviceId + '\n';
        Log.d("hexStr2Bytes", "---   --- 接收到数据：" + strbuf);
    }
    ...
    public UploadData Results_Data(ArrayList<String> hexlist) {
        if (hexlist.size()< 22){
            Log.d("blood_oxy", "------   血氧  size   ----- " + hexlist.size());
        }else{
            Log.d("blood_oxy", "------   血氧     ----- " + hexlist.get(18));
        }
...
//                + highstr + "  " + lowstr + "  " + oxy_vaulestr + "  " + information + '\n';
    ...
    private UploadData DataStr(UploadData uploadData, String ActiveType, String ActiveState, String SpasmState, String ECG, String blood, String blood_oxy) {
        ...
        Log.d("BT_CONNECTED", "------blood=    " + blood);
        uploadData.setBlood(bloodstr);

        //血氧状态
        ToastUtils.toast("--- 血氧仪     " + blood_oxy);
        switch (blood_oxy) {
            case "50":
                blood_oxystr = "血氧仪未连接";
                break;
...
//        ToastUtils.toast("--- 血氧仪     " + blood_oxystr);
